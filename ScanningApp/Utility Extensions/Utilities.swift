...
extension SCNMaterial {
  ...
  static func material(
    withDiffuse diffuse: Any?, respondsToLighting: Bool = false, isDoubleSided: Bool = true
  ) -> SCNMaterial {
...
extension SCNNode {
  ...
  open func hitTestWithSegment(
    from pointA: SIMD3<Float>, to pointB: SIMD3<Float>, options: [SCNHitTestOption: Any]? = nil
  ) -> [SCNHitTestResult] {
    if let options {
      ...
      return hitTestWithSegment(
        from: SCNVector3(pointA), to: SCNVector3(pointB), options: rawOptions)
    } else {
...
extension ARReferenceObject {
  func mergeInBackground(
    with otherReferenceObject: ARReferenceObject,
    completion: @escaping (ARReferenceObject?, Error?) -> Void
  ) {
