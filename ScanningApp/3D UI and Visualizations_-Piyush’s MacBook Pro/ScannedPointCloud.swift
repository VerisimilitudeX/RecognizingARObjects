...
class ScannedPointCloud: SCNNode, PointCloud {
  ...
  func scannedObjectPositionChanged(_ notification: Notification) {
    ...
    let boundingBox =
      scannedObject.boundingBox != nil ? scannedObject.boundingBox : scannedObject.ghostBoundingBox
    updateBoundingBox(boundingBox)
  }
  ...
  func updateOnEveryFrame() {
    ...
    self.preliminaryPointsNode.geometry = createVisualization(
      for: renderedPreliminaryPoints, color: .appLightYellow, size: 12)
  }
