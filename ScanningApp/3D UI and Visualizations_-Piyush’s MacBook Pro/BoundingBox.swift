...
class BoundingBox: SCNNode {
  ...
  private func createSides() {
    for position in BoundingBoxSide.Position.allCases {
      self.sides[position] = BoundingBoxSide(
        position, boundingBoxExtent: self.extent, color: self.color)
  ...
  func startSideDrag(screenPos: CGPoint) {
    ...
    for result in hitResults {
      if let side = result.node.parent as? BoundingBoxSide {
        ...
        let sideNormalInWorld = normalize(
          self.simdConvertVector(side.normal, to: nil) - self.simdConvertVector([0, 0, 0], to: nil))
        ...
        currentSideDrag = SideDrag(
          side: side, planeTransform: transform, beginWorldPos: self.simdWorldPosition,
          beginExtent: self.extent)
  ...
  func startSidePlaneDrag(screenPos: CGPoint) {
    ...
    for result in hitResults {
      if let side = result.node.parent as? BoundingBoxSide {
        ...
        let sideNormalInWorld = normalize(
          self.simdConvertVector(side.dragAxis.normal, to: nil)
            - self.simdConvertVector([0, 0, 0], to: nil))

        let planeNormalRay = Ray(
          origin: SIMD3<Float>(result.worldCoordinates), direction: sideNormalInWorld)
  ...
  private func tile(hitBy ray: Ray) -> (tile: Tile, hitLocation: SIMD3<Float>)? {
    ...
    let hitResults = self.sceneView.scene.rootNode.hitTestWithSegment(
      from: ray.origin, to: ray.origin + ray.direction,
      options: [
        .ignoreHiddenNodes: false,
