...
class ObjectOrigin: SCNNode {
  ...
  init(extent: SIMD3<Float>, _ sceneView: ARSCNView) {
    ...
    xAxis = ObjectOriginAxis(
      axis: .x, length: length, thickness: thickness, radius: radius,
      handleSize: handleSize)
    yAxis = ObjectOriginAxis(
      axis: .y, length: length, thickness: thickness, radius: radius,
      handleSize: handleSize)
    zAxis = ObjectOriginAxis(
      axis: .z, length: length, thickness: thickness, radius: radius,
      handleSize: handleSize)
    ...
    NotificationCenter.default.addObserver(
      self, selector: #selector(self.scanningStateChanged(_:)),
      name: Scan.stateChangedNotification, object: nil)
    NotificationCenter.default.addObserver(
      self, selector: #selector(self.boundingBoxExtentChanged(_:)),
  ...
  func set3DModel(_ url: URL?, extentForScaling: SIMD3<Float>? = nil) {
    ...
    if let url = url, let model = load3DModel(from: url) {
      ViewController.instance?.sceneView.prepare(
        [model],
    ...
    if let hitPos = sceneView.unprojectPointLocal(screenPos, ontoPlane: drag.planeTransform) {
      ...
      if isOutsideBoundingBox {
        NotificationCenter.default.post(
          name: ObjectOrigin.movedOutsideBoxNotification, object: self)
      }
    }
  }
