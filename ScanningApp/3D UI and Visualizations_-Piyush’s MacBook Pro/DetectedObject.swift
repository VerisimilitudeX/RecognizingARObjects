...
class DetectedObject: SCNNode {
  ...
  init(referenceObject: ARReferenceObject) {
    ...
    pointCloudVisualization = DetectedPointCloud(
      referenceObjectPointCloud: referenceObject.rawFeaturePoints,
  ...
  func updateVisualization(newTransform: float4x4, currentPointCloud: ARPointCloud) {
    ...
    if boundingBox == nil {
      ...
      let boundingBox = DetectedBoundingBox(
        points: referenceObject.rawFeaturePoints.points, scale: scale)
    ...
    self.isHidden = false
    self.detectedObjectVisualizationTimer = Timer.scheduledTimer(
