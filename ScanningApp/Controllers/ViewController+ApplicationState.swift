...
extension ViewController {
  ...
  func scanningStateChanged(_ notification: Notification) {
    guard self.state == .scanning, let scan = notification.object as? Scan, scan === self.scan
    else { return }
    ...
    DispatchQueue.main.async {
      ...
      case .scanning:
        self.displayInstruction(
          Message(
            "Scan the object from all sides that you are "
      ...
      case .adjustingOrigin:
        ...
        self.displayInstruction(
          Message("Adjust origin using gestures.\n" + "You can load a *.usdz 3D model overlay."))
