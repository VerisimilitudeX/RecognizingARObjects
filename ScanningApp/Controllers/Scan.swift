...
class Scan {
    ...
    set {
      ...
      case .scanning where stateValue == .defineBoundingBox && !isReasonablySized,
        ...
        ViewController.instance?.showAlert(
          title: title, message: message, buttonTitle: "Yes", showCancel: true
        ) { _ in
  ...
  private func applicationStateChanged(_ notification: Notification) {
    guard
      let appState = notification.userInfo?[ViewController.appStateUserInfoKey]
        as? ViewController.State
    else { return }
    switch appState {
    case .scanning:
    ...
    if state == .defineBoundingBox || state == .scanning {
      ...
      case .began:
        if gesture.numberOfTouches == 2 {
          scannedObject.boundingBox?.startGroundPlaneDrag(
            screenPos: gesture.offsetLocation(in: sceneView))
        }
      case .changed where gesture.isThresholdExceeded:
        if gesture.numberOfTouches == 2 {
          scannedObject.boundingBox?.updateGroundPlaneDrag(
            screenPos: gesture.offsetLocation(in: sceneView))
        }
      case .changed:
        break
  ...
  func updateOnEveryFrame(_ frame: ARFrame) {
    ...
    if state == .ready || state == .defineBoundingBox || state == .scanning {
      ...
      if let lightEstimate = frame.lightEstimate, lightEstimate.ambientIntensity < 500,
        !hasWarnedAboutLowLight, isFirstScan
      {
  ...
  var isReasonablySized: Bool {
    ...
    if validSizeRange.contains(boundingBox.extent.x)
  ...
  func createReferenceObject(
    completionHandler creationFinished: @escaping (ARReferenceObject?) -> Void
  ) {
      ...
      completionHandler: { object, error in
        if let referenceObject = object {
          ...
          if let referenceObjectToMerge = ViewController.instance?.referenceObjectToMerge {
            ...
            ViewController.instance?.showAlert(
              title: "", message: "Merging previous scan into this scan...", buttonTitle: nil)

            // Try to merge the object which was just scanned with the existing one.
            self.scannedReferenceObject?.mergeInBackground(
              with: referenceObjectToMerge,
              completion: { (mergedObject, error) in
                ...
                if let mergedObject = mergedObject {
                  ...
                  let previousScan = UIAlertAction(title: "Use Previous Scan", style: .default) {
                    _ in
                    self.scannedReferenceObject = referenceObjectToMerge
                    creationFinished(self.scannedReferenceObject)
                  }
                  ViewController.instance?.showAlert(
                    title: "Merge failed", message: message, actions: [thisScan, previousScan])
                }
              })
          } else {
            creationFinished(self.scannedReferenceObject)
